...
const FileListTable: React.FC<FileListTableProps> = ({ files, handleFileDownload }) => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (activeDetailIndex !== null) {
            ...
            if (activeDetails && !activeDetails.contains(event.target as Node)) {
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {files?.map((file, index) => (
                                ...
                                >
                                    ...
                                    <td className="w-[50%] md:w-3/5 px-2.5 md:px-4 py-2.5 md:py-4">
                                        <div className="relative flex items-center">
                                            ...
                                            {activeDetailIndex === index && (
                                                <div ref={(el: any) => (detailsRef.current[index] = el)} className="absolute hidden group-hover:block z-20 bg-gray-800 p-2.5 md:p-4 rounded-xl shadow-2xl -top-2 left-1/2 ml-4 w-36 md:w-72 border border-gray-700">
                                    ...
                                    <td className="hidden md:table-cell w-[50%] md:w-[30%] px-4 py-4">
                                        {file.progress > 0 && (
                                    ...
                                    <td className="hidden w-20 md:w-[120px] px-4 py-4">
                                        <button
                                            ...
                                            className={`inline-flex items-center justify-center px-3 py-1.5 text-[8px] md:text-xs font-medium rounded-md 
                                               text-white bg-blue-600 hover:bg-blue-700 
                                               disabled:bg-gray-500 disabled:cursor-not-allowed
                                               transition-colors duration-200 w-full`}
                                        >
                                            {downloadingStates[index] ? 'Processing...' : 'Download'}
                                        </button>
                                    </td>
                                </tr>
                            ))}
